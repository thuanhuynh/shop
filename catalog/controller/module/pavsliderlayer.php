...
class ControllermodulePavsliderlayer extends Controller {
	...
	public function index( $setting ) { 
		...
		$this->document->addScript("catalog/view/javascript/layerslider/raphael-min.js");
		$this->document->addScript("catalog/view/javascript/layerslider/jquery.easing.js");
		$this->document->addScript("catalog/view/javascript/layerslider/iview.js");
		...
		foreach( $sliders as $key => $slider ){
			...
			if( $sliderGroup['params']['image_cropping']) { 
				if( $slider['params']['slider_thumbnail'] ) {
					$slider['thumbnail'] = $model->resize( $slider['params']['slider_thumbnail'], $sliderGroup['params']['thumbnail_width'], $sliderGroup['params']['thumbnail_height'],'w'); 
				}else {
					$slider['thumbnail'] = $model->resize( $slider['image'], $sliderGroup['params']['thumbnail_width'], $sliderGroup['params']['thumbnail_height'],'w'); 
				}
			} else {
				if( $slider['params']['slider_thumbnail'] ) {
